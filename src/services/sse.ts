 ...
 * Handles real-time event streaming from the backend for Client Dashboard
 *
 ...
 *
 * For Client Dashboard: Events are automatically scoped to the authenticated restaurant
 */

import { env } from "@/config/env";
import { ENDPOINTS } from "@/lib/api/endpoints";
import { api, getAccessToken, getRestaurantId } from "@/lib/api/client";
import type {
  SSEEvent,
  SSEConnectionStats,
  SSEEscalationType,
  SSETriggerEscalationRequest,
 ...
 *
 * Note: For Client Dashboard, the backend automatically scopes events
 * to the authenticated restaurant, so no restaurant_id filter is needed
 */
export const connectToSSE = (options: SSEConnectOptions): EventSource | null => {
  ...
  // Restaurant ID is auto-detected from token on the backend for client dashboard users
  const restaurantId = getRestaurantId();
  let url = `${env.API_URL}${ENDPOINTS.SSE.STREAM}?token=${encodeURIComponent(token)}`;

  // Add restaurant_id as a hint (backend will validate against token)
  if (restaurantId) {
 ...
 */
export const disconnectFromSSE = (eventSource: EventSource | null): void => {
  if (eventSource) {
    eventSource.close();
 ...
 *
 * Note: This is typically called by the backend during AI calls,
 * but can be used for testing or manual escalation triggers
 */
export const triggerEscalation = async (
  ...
  const request: SSETriggerEscalationRequest = { type, data };

  const response = await api.post(ENDPOINTS.SSE.ESCALATION(restaurantId), request);
 ...
 * GET /api/v1/sse/events/stats
 *
 * Note: This endpoint may be admin-only
 */
